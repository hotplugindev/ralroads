...
class _ChallengeDetailScreenState extends State<ChallengeDetailScreen> {
  ...
  > _loadData() async {
    final challenge = await widget.repositories.challenges.getChallenge(widget.challengeId);
    if (challenge == null) {
      throw Exception('Challenge not found.');
    }

    final segment = await widget.repositories.segments.getSegment(challenge.segmentId);
    if (segment == null) {
    ...
    final geomList = await (widget.repositories.segments.database.select(
    ...
    final localProfile = await widget.repositories.profiles.getCurrentLocalProfile();

    return (challenge, segment, version, points, attempts, localProfile);
  }

  Future<String> _determineTrustLabel(SegmentAttempt attempt, Profile? localProfile) async {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Rally Challenge Details', style: TextStyle(fontWeight: FontWeight.bold)),
      ),
      body: FutureBuilder<
        (
          Challenge,
      ...
      >(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const LoadingState(label: 'Loading challenge details...');
          }
          ...
          if (data == null) {
            return const ErrorState(message: 'Challenge details not found.');
          }

          final (challenge, segment, version, points, allAttempts, localProfile) = data;

          // Filter attempts to only match the challenge window
          final challengeAttempts = allAttempts.where((attempt) {
            if (challenge.startsAt != null && attempt.startedAt.isBefore(challenge.startsAt!)) {
              return false;
            }
            if (challenge.endsAt != null && attempt.startedAt.isAfter(challenge.endsAt!)) {
              return false;
            }
            return true;
          }).toList();
          ...
          final validAttempts = challengeAttempts.where((a) => a.finishedAt != null).toList();
              ...
              children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Column(
                              ...
                              children: [
                                Text('Time Remaining', style: theme.textTheme.bodySmall),
                                const SizedBox(height: 2),
                                Text(
                                  timeLabel,
                                  style: theme.textTheme.bodyMedium?.copyWith(
                ...
                if (isActive)
                  PrimaryActionCard(
                    ...
                    subtitle: 'Launch rally-assistant and complete the segment within target speed limits to qualify.',
                ...
                if (validAttempts.isEmpty)
                  const EmptyState(
                    ...
                    message: 'Drive this segment to post the first time on the leaderboard!',
                  )
                else
                  ListView.builder(
                    shrinkWrap: true,
                            ...
                            child: Padding(
                              ...
                              child: Row(
                                children: [
                                        ...
                                        Row(
                                          children: [
                                            Text(
                                              ...
                                              style: theme.textTheme.titleMedium?.copyWith(
                                                fontWeight: FontWeight.bold,
                                                color: isLocal ? scheme.primary : null,
                                              ),
                                            ),
                                  ...
                                  if (isLocal && attempt.status == 'valid_clean')
                                    IconButton(
                                      icon: const Icon(Icons.share_outlined),
                                      tooltip: 'Share Attempt',
                                      onPressed: () => _shareAttempt(context, attempt.id),
                                    ),
                                ],
                              ),
                            ),
  ...
  Future<void> _shareAttempt(BuildContext context, String attemptId) async {
    final rooms = await widget.repositories.groups.database.select(
      widget.repositories.groups.database.rooms,
    ).get();
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  leading: Icon(room.type == 'group' ? Icons.groups_outlined : Icons.person_outline),
                  title: Text(room.name ?? room.matrixRoomId),
                  onTap: () async {
                    Navigator.of(ctx).pop();
                    await widget.socialController.shareAttempt(room.matrixRoomId, attemptId);
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Attempt sharing queued.')),
                      );
                    }
                  },
                );
  ...
  Future<void> _shareChallenge(BuildContext context, Challenge challenge) async {
    // If the challenge is already shared to a room, copy details or notify user
    if (challenge.roomId != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Challenge is active in group: ${challenge.roomId}')),
      );
    }
  }
}

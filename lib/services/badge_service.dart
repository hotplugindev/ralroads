...
class BadgeService {
  ...
  Future<void> checkAndAwardBadges(String profileId) async {
    final attempts = await (database.select(database.segmentAttempts)
          ..where((row) => row.profileId.equals(profileId)))
        .get();

    final cleanAttempts = attempts.where((a) => a.status == 'validClean' || a.status == 'valid_clean').toList();
  ...
  Future<void> _awardBadge({
    ...
    final exists = await (database.select(database.localNotifications)

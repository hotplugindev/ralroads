  ...
  Future<void> _deleteRegion(maplibre.OfflineRegion region) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete cached map for "$name"?'),
        actions: [
          TextButton(
          ...
          if (status is maplibre.InProgress) {
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Offline map for "${route.name}" downloaded!')),
              );
              _loadData();
            }
                      ...
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          ...
                          final dateStr = region.metadata['createdAt'] != null
                              ? DateTime.parse(region.metadata['createdAt'])
                                  .toLocal()
                                  .toString()
                                  .split('.')
                                  .first
                              : 'Unknown Date';
                          final styleStr = region.definition.mapStyleUrl.split('/').last;
                    ...
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          ...
                          final isDownloading = _downloadingRoutes[route.id] ?? false;

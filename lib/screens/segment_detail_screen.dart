...
class SegmentDetailScreen extends StatefulWidget {
  ...
  final dynamic session; // dynamic to avoid strong coupling with session controller imports
...
class _SegmentDetailScreenState extends State<SegmentDetailScreen> {
  late Future<(ChallengeSegment, SegmentVersion, List<RoutePoint>, List<SegmentAttempt>)> _future;
  ...
  Future<(ChallengeSegment, SegmentVersion, List<RoutePoint>, List<SegmentAttempt>)> _loadData() async {
    final segment = await widget.repositories.segments.getSegment(widget.segmentId);
    if (segment == null) {
    ...
    final attempts = await widget.repositories.attempts.listAttemptsForSegment(widget.segmentId);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: FutureBuilder<(ChallengeSegment, SegmentVersion, List<RoutePoint>, List<SegmentAttempt>)>(
        future: _future,
              ...
              if (attempts.isEmpty)
                ...
                ...attempts.map((attempt) {
                  final durationStr = attempt.finishedAt != null
                      ? formatDuration(attempt.finishedAt!.difference(attempt.startedAt))
                      : 'DNF';
                  return Card(
                    elevation: 0,
                    child: ListTile(
                      leading: Icon(
                        ...
                        color: attempt.status == 'valid_clean' ? Colors.green : Colors.orange,
                      ),
                      title: Text(
                        'Time: $durationStr',
                        style: const TextStyle(fontWeight: FontWeight.bold),
  ...
  Future<void> _startAttempt(ChallengeSegment segment, List<RoutePoint> points) async {

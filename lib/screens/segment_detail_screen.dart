...
class _SegmentDetailScreenState extends State<SegmentDetailScreen> {
  late Future<
    (ChallengeSegment, SegmentVersion, List<RoutePoint>, List<SegmentAttempt>, Profile?)
  >
  _future;
      ...
      >(
        ...
        builder: (context, snapshot) {
          ...
          sortedAttempts.sort((a, b) {
            if (a.status == 'valid_clean' && b.status != 'valid_clean') return -1;
            if (a.status != 'valid_clean' && b.status == 'valid_clean') return 1;
          ...
          return RalRoadsPage(
            ...
            actions: [
              if (socialController != null)
                IconButton(
                  ...
                  onPressed: () => _shareSegment(context, socialController, segment.id),
                ),
              IconButton(
            ...
            children: [
              ...
              SectionHeader(
                title: 'Leaderboard',
                trailing: StatusChip(label: '${sortedAttempts.length} times'),
              ),
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            children: [
                              ...
                              // Attempt actions
                              if (isLocal && socialController != null && attempt.status == 'valid_clean')
                                IconButton(
                                  icon: const Icon(Icons.share_outlined),
                                  tooltip: 'Share Attempt',
                                  onPressed: () => _shareAttempt(context, socialController, attempt.id),
                                ),
                              if (!isLocal)
                                IconButton(
                                  icon: const Icon(Icons.report_problem_outlined, color: Colors.grey),
                                  tooltip: 'Report Content',
                                  onPressed: () => _reportAttempt(context, socialController, attempt.id),
                                ),
                            ],
                          ),
                        ),
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  onTap: () async {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Segment sharing queued.')),
                      );
                    }
                  },
                );

  ...
  group('HUD Navigation ETA and Progress Calculations', () {
    test('calculates correct remaining distance and progress percentage', () {
      ...
      final remainingDistanceMeters = routePoints.isNotEmpty
          ? (routePoints.last.distanceFromStart - distanceAlongRoute).clamp(
              0.0,
              double.infinity,
            )
          : 0.0;

      final totalDistance = routePoints.isNotEmpty
          ? routePoints.last.distanceFromStart
          : 1.0;
      final progressPercentage = (distanceAlongRoute / totalDistance).clamp(
        0.0,
        1.0,
      );
    ...
    test(
      'calculates correct remaining duration incorporating speed limit changes',
      () {
        final routePoints = [
          const RoutePoint(lat: 0.0, lon: 0.0, distanceFromStart: 0.0),

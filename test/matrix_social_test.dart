...
import 'package:ralroads/online/matrix/matrix_outbox_worker.dart';
...
class FakeMatrixClient implements matrix_sdk.Client {
  ...
  dynamic noSuchMethod(Invocation invocation) {
    if (invocation.memberName == #createRoom) {
      ...
      final inviteArg = invocation.namedArguments[const Symbol('invite')] as List<dynamic>?;
      if (inviteArg != null) {
    ...
    if (invocation.memberName == #setRoomStateWithKey) {
      ...
      final stateKey = invocation.positionalArguments[2] as String;
      final content = invocation.positionalArguments[3] as Map<dynamic, dynamic>;
...
void main() {
  ...
  group('MatrixSocialController Friend Tests', () {
    test('sendFriendRequest enqueues outgoing event and upserts request locally', () async {
      ...
      expect(clientService._fakeClient.invites[roomId], contains('@bob:matrix.org'));
    ...
    test('acceptFriendRequest updates state to accepted and enqueues event', () async {
      // 1. Create profiles to satisfy foreign keys
      await repositories.profiles.createOrUpdateLocalProfile(
        const LocalProfileInput(id: 'matrix--bob-matrix-org', displayName: 'Bob', matrixUserId: '@bob:matrix.org'),
      );
      await repositories.profiles.createOrUpdateLocalProfile(
        const LocalProfileInput(id: 'matrix--alice-matrix-org', displayName: 'Alice', matrixUserId: '@alice:matrix.org'),
      );
      ...
      final req = await (database.select(database.friendRequests)
            ..where((row) => row.id.equals(requestId)))
          .getSingleOrNull();
      ...
      expect(clientService._fakeClient.joinedRooms, contains('!room-123:matrix.org'));

      // Check request state is updated
      final updatedReq = await (database.select(database.friendRequests)
            ..where((row) => row.id.equals(requestId)))
          .getSingleOrNull();
      expect(updatedReq?.state, 'accepted');

      // Check friendship is saved
      final friendship = await (database.select(database.friendships)..limit(1)).getSingle();
  ...
  group('MatrixSocialController Group Tests', () {
    test('createGroup creates room, sets E2EE, and saves local group', () async {
      ...
      final roomId = clientService._fakeClient.createdRooms.first;
      expect(clientService._fakeClient.roomEncryptions[roomId], 'm.megolm.v1.aes-sha2');

      // Check database
      final group = await (database.select(database.groups)..limit(1)).getSingle();
  ...
  group('MatrixSocialController Sharing & Placeholder Resolver Tests', () {
    test('shareSegment encrypts and queue uploads', () async {
      ...
      await repositories.profiles.createOrUpdateLocalProfile(
        const LocalProfileInput(id: 'matrix--alice-matrix-org', displayName: 'Alice'),
      );
      ...
      final payload = jsonDecode(events.first.payloadJson) as Map<String, dynamic>;
